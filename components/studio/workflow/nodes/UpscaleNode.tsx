import React, { memo } from 'react';
import { Handle, Position, NodeProps } from '@xyflow/react';
import { Maximize2, Loader2, CheckCircle } from 'lucide-react';
import { cn } from '../../../../lib/utils';

export const UpscaleNode = memo(({ id, data, selected }: NodeProps) => {
  const status = data.status || 'idle';

  const handleParameterChange = (key: string, value: any) => {
    data.parameters = { ...data.parameters, [key]: value };
    data.onChange?.(data.parameters);
  };

  return (
    <div
      className={cn(
        'rounded-xl border-2 bg-white dark:bg-zinc-900 transition-all duration-200 min-w-[280px]',
        status === 'running' && 'border-blue-500 ring-2 ring-blue-500/20',
        status === 'completed' && 'border-emerald-500 ring-2 ring-emerald-500/20',
        status === 'error' && 'border-red-500 ring-2 ring-red-500/20',
        status === 'idle' && 'border-cyan-300 dark:border-cyan-700',
        selected && 'ring-4 ring-teal-500/30'
      )}
    >
      {/* Header */}
      <div className="flex items-center justify-between p-3 border-b border-zinc-200 dark:border-zinc-800 bg-cyan-50 dark:bg-cyan-950/30">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-cyan-100 dark:bg-cyan-900/50 text-cyan-600 dark:text-cyan-400">
            <Maximize2 size={18} />
          </div>
          <div>
            <div className="text-sm font-bold text-zinc-900 dark:text-white">
              Upscale Image
            </div>
            {status !== 'idle' && (
              <div className="text-[10px] text-zinc-500 dark:text-zinc-400">
                {status === 'running' && 'Upscaling...'}
                {status === 'completed' && 'Complete'}
                {status === 'error' && 'Error'}
              </div>
            )}
          </div>
        </div>
        {status === 'running' && <Loader2 size={16} className="animate-spin text-blue-500" />}
        {status === 'completed' && <CheckCircle size={16} className="text-emerald-500" />}
      </div>

      {/* Input Handle */}
      <Handle
        type="target"
        position={Position.Left}
        id="image"
        className="w-3 h-3 bg-blue-500 border-2 border-white dark:border-zinc-900"
        style={{ top: '50%' }}
      />

      {/* Content */}
      <div className="p-3 space-y-3">
        {/* Scale Factor */}
        <div>
          <label className="block text-xs font-medium text-zinc-700 dark:text-zinc-300 mb-1">
            Scale Factor
          </label>
          <select
            value={data.parameters?.scale || '2'}
            onChange={(e) => handleParameterChange('scale', e.target.value)}
            className="w-full px-3 py-2 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg text-sm text-zinc-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-cyan-500/20"
          >
            <option value="2">2x (Double)</option>
            <option value="4">4x (Quadruple)</option>
            <option value="8">8x (8 times)</option>
          </select>
        </div>

        {/* Model Selection */}
        <div>
          <label className="block text-xs font-medium text-zinc-700 dark:text-zinc-300 mb-1">
            Model
          </label>
          <select
            value={data.parameters?.model || 'clarity-upscaler'}
            onChange={(e) => handleParameterChange('model', e.target.value)}
            className="w-full px-3 py-2 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg text-sm text-zinc-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-cyan-500/20"
          >
            <option value="clarity-upscaler">Clarity Upscaler</option>
            <option value="creative-upscaler">Creative Upscaler</option>
          </select>
        </div>

        {/* Creativity Slider (for creative-upscaler) */}
        {data.parameters?.model === 'creative-upscaler' && (
          <div>
            <label className="flex items-center justify-between text-xs font-medium text-zinc-700 dark:text-zinc-300 mb-1">
              <span>Creativity</span>
              <span className="text-cyan-600 dark:text-cyan-400">
                {data.parameters?.creativity || 5}
              </span>
            </label>
            <input
              type="range"
              min="0"
              max="10"
              step="1"
              value={data.parameters?.creativity || 5}
              onChange={(e) => handleParameterChange('creativity', parseInt(e.target.value))}
              className="w-full h-2 bg-zinc-200 dark:bg-zinc-800 rounded-lg appearance-none cursor-pointer accent-cyan-600"
            />
            <div className="flex justify-between text-[10px] text-zinc-400 mt-1">
              <span>Faithful</span>
              <span>Creative</span>
            </div>
          </div>
        )}

        {/* Preview */}
        {data.output?.imageUrl && (
          <div className="mt-3">
            <img
              src={data.output.imageUrl}
              alt="Upscaled"
              className="w-full h-40 object-cover rounded-lg border border-zinc-200 dark:border-zinc-800"
            />
          </div>
        )}

        {/* Error */}
        {data.error && (
          <div className="p-2 rounded-lg bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-800">
            <p className="text-xs text-red-600 dark:text-red-400">{data.error}</p>
          </div>
        )}
      </div>

      {/* Output Handle */}
      <Handle
        type="source"
        position={Position.Right}
        id="image"
        className="w-3 h-3 bg-blue-500 border-2 border-white dark:border-zinc-900"
        style={{ top: '50%' }}
      />
    </div>
  );
});

UpscaleNode.displayName = 'UpscaleNode';
