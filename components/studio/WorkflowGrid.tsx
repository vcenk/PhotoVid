  ...
  if (!selectedIndustry) {
    return (
      <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-zinc-200">
        <h3 className="text-xl font-medium text-zinc-900 mb-2">Select an Industry</h3>
        <p className="text-zinc-500">Choose an industry above to see specialized workflows.</p>
      </div>
    );
  }
      ...
      <AnimatePresence mode="popLayout">
        {workflows.map((workflow) => (
          <motion.div
            ...
            className="group cursor-pointer bg-white rounded-2xl border border-zinc-200 overflow-hidden hover:shadow-xl hover:border-indigo-500/30 transition-all duration-300"
          >
            {/* Preview Image Area */}
            <div className="h-40 bg-zinc-100 flex items-center justify-center relative overflow-hidden">
               {workflow.previewImage ? (
                 <img src={workflow.previewImage} alt={workflow.name} className="w-full h-full object-cover" />
               ) : (
                 <div className="p-4 bg-indigo-50 rounded-full text-indigo-500 group-hover:scale-110 transition-transform duration-500">
                    <workflow.icon size={32} />
                 </div>
               )}
               <div className="absolute top-3 right-3 bg-white/90 backdrop-blur px-2 py-1 rounded-full text-xs font-medium text-zinc-600 flex items-center gap-1 shadow-sm">
