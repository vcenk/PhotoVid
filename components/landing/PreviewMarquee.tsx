...
const PreviewCard: React.FC<{ item: PreviewItem; offset: number }> = ({ item, offset }) => (
  <motion.div
    ...
    className="group relative w-[240px] md:w-[320px] aspect-[16/10] flex-shrink-0 rounded-2xl overflow-hidden bg-white border border-zinc-200 shadow-xl transition-all duration-500 cursor-pointer"
  >
    <img 
      src={item.image} 
      alt={item.label}
    ...
    <div className="absolute inset-0 p-4 flex flex-col justify-end">
      <div className="flex gap-2 items-center mb-2">
        <span className="px-2 py-0.5 rounded-full bg-white/80 border border-zinc-200 text-[8px] md:text-[9px] font-bold uppercase tracking-widest text-zinc-900 shadow-sm">
          {item.type}
        </span>
        <span className="px-2 py-0.5 rounded-full bg-white/80 border border-zinc-200 text-[8px] md:text-[9px] font-medium text-zinc-500 shadow-sm">
          {item.ratio}
        </span>
      </div>
      <h4 className="text-sm md:text-base font-semibold text-zinc-900 tracking-tight">
        {item.label}
      </h4>
    </div>
  </motion.div>
...
export const PreviewMarquee: React.FC = () => {
  ...
  return (
    <section className="relative py-16 md:py-24 overflow-hidden select-none bg-zinc-50 border-t border-zinc-200">
      <div className="container mx-auto px-6 mb-12">
        <h3 className="text-center text-[10px] md:text-xs font-bold uppercase tracking-[0.4em] text-zinc-400">
