...
const SHOWCASE_ITEMS_TOP = [
  ...
  {
    id: 5,
    // UPDATED: Verified Cyberpunk Street Image
...
const SHOWCASE_ITEMS_BOTTOM = [
  ...
  {
    id: 15,
    // UPDATED: Verified Gaming Setup Image
  ...
  return (
    ...
    >
      ...
      <div className="relative z-0 space-y-8 rotate-[-2deg] scale-110">
        ...
        <div className="flex gap-6 animate-marquee-left w-max">
          {topRow.map((item, idx) => (
            <div 
              key={`top-${item.id}-${idx}`} 
              className="relative w-[350px] aspect-[4/3] rounded-2xl overflow-hidden group flex-shrink-0 cursor-pointer border border-white/10"
            >
              <img 
                src={item.image} 
                alt={item.title}
              ...
              <div className="absolute bottom-6 left-6 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <span className="text-[10px] font-bold uppercase tracking-widest text-indigo-400 block mb-1">{item.tag}</span>
      ...
      <div className="absolute inset-0 z-20 flex flex-col items-center justify-center pointer-events-none">
         <h2 className="text-6xl md:text-9xl font-black text-transparent stroke-text tracking-tighter opacity-30 select-none">
           SHOWCASE
         </h2>
