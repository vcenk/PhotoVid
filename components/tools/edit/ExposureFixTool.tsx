...
import {
    ...
    Image as ImageIcon, AlertCircle, Sun, BookmarkPlus, Check
} from 'lucide-react';
import { NavigationRail } from '../../dashboard/navigation/NavigationRail';
import { AssetProvider, useAssets } from '@/lib/store/contexts/AssetContext';
import { generateExposureFix, isFalConfigured } from '@/lib/api/toolGeneration';
import { downloadFile } from '@/lib/utils';
import { BeforeAfterSlider } from '@/components/common/BeforeAfterSlider';

const ExposureFixToolInner: React.FC = () => {
    ...
    const handleGenerate = async () => {
        ...
        if (!isFalConfigured()) {
            ...
            setTimeout(async () => {
                ...
                const mockUrl = 'https://images.unsplash.com/photo-1600566753190-17f0baa2a6c3?w=1200&h=800&fit=crop';
                setResultImage(mockUrl);
                try { await addAsset(mockUrl, 'image', 'Exposure Fixed'); setSavedToLibrary(true); } catch {}
        ...
        try {
            const resultUrl = await generateExposureFix(
                uploadedImage,
                (progress) => setGenerationProgress(progress)
            );
            setResultImage(resultUrl);
            try { await addAsset(resultUrl, 'image', 'Exposure Fixed'); setSavedToLibrary(true); } catch {}
        } catch (err) {
            console.error('Exposure fix error:', err);
            setError(err instanceof Error ? err.message : 'Exposure fix failed.');
        } finally {
            setIsGenerating(false);
        }
    };
                ...
                <div className="h-16 flex items-center justify-between px-6 border-b border-white/5 bg-[#111113]">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h1 className="text-lg font-semibold text-white">Exposure Fix</h1>
                                <p className="text-xs text-zinc-500">Recover over/underexposed areas</p>
                            </div>
                        </div>
                    </div>
                    ...
                    {resultImage && (
                        <div className="flex items-center gap-2">
                            ...
                            {savedToLibrary ? (
                                ...
                                <button onClick={async () => { if (resultImage) { try { await addAsset(resultImage, 'image', 'Exposure Fixed'); setSavedToLibrary(true); } catch {} } }} className="px-4 py-2 text-sm font-medium text-zinc-400 hover:text-white bg-white/5 hover:bg-white/10 rounded-xl transition-colors flex items-center gap-2">
                                    <BookmarkPlus size={16} />Save
                                </button>
                            )}
                            <button onClick={() => resultImage && downloadFile(resultImage, `exposure-fixed-${Date.now()}.jpg`)} className="px-4 py-2 text-sm font-medium text-white bg-yellow-600 hover:bg-yellow-500 rounded-xl transition-colors flex items-center gap-2">
                    ...
                    <div className="w-80 flex-shrink-0 border-r border-white/5 bg-[#111113] flex flex-col">
                        <div className="flex-1 overflow-y-auto p-5 space-y-6">
                            ...
                            <div className="p-4 rounded-xl bg-yellow-500/5 border border-yellow-500/10">
                                <div className="flex items-start gap-3">
                                    <Sun size={18} className="text-yellow-400 mt-0.5" />
                                    <div>
                                        <p className="text-sm font-medium text-yellow-300">AI Exposure Recovery</p>
                                        <p className="text-xs text-yellow-300/70 mt-1">Recovers details from over or underexposed property photos, especially window views.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="p-5 border-t border-white/5">
                            <button
                                ...
                                className={`w-full py-4 rounded-xl font-semibold text-sm transition-all flex items-center justify-center gap-2 ${uploadedImage && !isGenerating ? 'bg-gradient-to-r from-yellow-500 to-amber-500 hover:from-yellow-400 hover:to-amber-400 text-white shadow-lg shadow-yellow-500/25' : 'bg-white/5 text-zinc-600 cursor-not-allowed'}`}
                            >
                                {isGenerating ? <><Loader2 size={18} className="animate-spin" />Processing {generationProgress}%</> : <><Sun size={18} />Fix Exposure</>}
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 flex items-center justify-center p-8 bg-[#0a0a0b]">
                        {error ? (
                            ...
                            <div className="text-center">
                                ...
                                <p className="text-zinc-500 text-lg">Upload an image to fix</p>
                            </div>
                        ) : isGenerating ? (
                            <div className="text-center">
                                ...
                                <p className="text-zinc-400 font-medium text-lg">Fixing exposure...</p>
                                <p className="text-zinc-600 text-sm mt-2">Recovering details</p>
                            </div>
                            ...
                            <div className="relative rounded-2xl overflow-hidden shadow-2xl">
                                ...
                                <div className="absolute inset-0 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                                    <div className="text-center text-white">
                                        <Sun size={36} className="mx-auto mb-3 opacity-80" />
                                        <p className="text-lg font-medium">Ready to fix</p>
                                        <p className="text-sm text-white/60 mt-1">Recover exposure details</p>
                                    </div>
                                </div>
                            </div>
                        )}
...
export const ExposureFixTool: React.FC = () => (
    <AssetProvider>
        <ExposureFixToolInner />
    </AssetProvider>
);
