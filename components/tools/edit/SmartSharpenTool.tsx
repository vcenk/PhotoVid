...
import {
    ...
    Image as ImageIcon, AlertCircle, Focus, BookmarkPlus, Check
} from 'lucide-react';
import { NavigationRail } from '../../dashboard/navigation/NavigationRail';
import { AssetProvider, useAssets } from '@/lib/store/contexts/AssetContext';
import { generateSharpen, isFalConfigured } from '@/lib/api/toolGeneration';
import { downloadFile } from '@/lib/utils';
import { BeforeAfterSlider } from '@/components/common/BeforeAfterSlider';

const SmartSharpenToolInner: React.FC = () => {
    ...
    const handleGenerate = async () => {
        ...
        if (!isFalConfigured()) {
            ...
            setTimeout(async () => {
                ...
                try { await addAsset(mockUrl, 'image', 'Sharpened'); setSavedToLibrary(true); } catch {}
        ...
        try {
            const resultUrl = await generateSharpen(
                uploadedImage,
                (progress) => setGenerationProgress(progress)
            );
            setResultImage(resultUrl);
            try { await addAsset(resultUrl, 'image', 'Sharpened'); setSavedToLibrary(true); } catch {}
        } catch (err) {
                ...
                <div className="h-16 flex items-center justify-between px-6 border-b border-white/5 bg-[#111113]">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h1 className="text-lg font-semibold text-white">Smart Sharpen</h1>
                                <p className="text-xs text-zinc-500">AI detail recovery for property photos</p>
                            </div>
                        </div>
                    </div>
                    ...
                    {resultImage && (
                        <div className="flex items-center gap-2">
                            ...
                            {savedToLibrary ? (
                                ...
                                <button onClick={async () => { if (resultImage) { try { await addAsset(resultImage, 'image', 'Sharpened'); setSavedToLibrary(true); } catch {} } }} className="px-4 py-2 text-sm font-medium text-zinc-400 hover:text-white bg-white/5 hover:bg-white/10 rounded-xl transition-colors flex items-center gap-2">
                                    <BookmarkPlus size={16} />Save
                                </button>
                            )}
                            <button onClick={() => resultImage && downloadFile(resultImage, `sharpened-${Date.now()}.jpg`)} className="px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-500 rounded-xl transition-colors flex items-center gap-2">
                    ...
                    <div className="w-80 flex-shrink-0 border-r border-white/5 bg-[#111113] flex flex-col">
                        <div className="flex-1 overflow-y-auto p-5 space-y-6">
                            ...
                            <div className="p-4 rounded-xl bg-blue-500/5 border border-blue-500/10">
                                <div className="flex items-start gap-3">
                                    <Focus size={18} className="text-blue-400 mt-0.5" />
                                    <div>
                                        <p className="text-sm font-medium text-blue-300">AI Sharpening</p>
                                        <p className="text-xs text-blue-300/70 mt-1">Recovers fine details and enhances clarity in soft or blurry property photos.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="p-5 border-t border-white/5">
                            <button
                                ...
                                className={`w-full py-4 rounded-xl font-semibold text-sm transition-all flex items-center justify-center gap-2 ${uploadedImage && !isGenerating ? 'bg-gradient-to-r from-blue-500 to-cyan-500 hover:from-blue-400 hover:to-cyan-400 text-white shadow-lg shadow-blue-500/25' : 'bg-white/5 text-zinc-600 cursor-not-allowed'}`}
                            >
                                {isGenerating ? <><Loader2 size={18} className="animate-spin" />Sharpening {generationProgress}%</> : <><Focus size={18} />Sharpen Image</>}
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 flex items-center justify-center p-8 bg-[#0a0a0b]">
                        {error ? (
                            <div className="text-center max-w-md">
                                ...
                                <p className="text-red-400 text-lg font-medium mb-2">Sharpening Failed</p>
                            ...
                            <div className="text-center">
                                ...
                                <p className="text-zinc-500 text-lg">Upload an image to sharpen</p>
                            </div>
                        ) : isGenerating ? (
                            <div className="text-center">
                                ...
                                <p className="text-zinc-400 font-medium text-lg">Sharpening image...</p>
                                <p className="text-zinc-600 text-sm mt-2">AI is recovering details</p>
                            </div>
                            ...
                            <div className="relative rounded-2xl overflow-hidden shadow-2xl">
                                ...
                                <div className="absolute inset-0 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                                    <div className="text-center text-white">
                                        <Focus size={36} className="mx-auto mb-3 opacity-80" />
                                        <p className="text-lg font-medium">Ready to sharpen</p>
                                        <p className="text-sm text-white/60 mt-1">Enhance clarity and detail</p>
                                    </div>
                                </div>
                            </div>
                        )}
...
export const SmartSharpenTool: React.FC = () => (
    <AssetProvider>
        <SmartSharpenToolInner />
    </AssetProvider>
);
