...
import {
    ...
    AlertCircle, BadgeCheck, Car, Check
} from 'lucide-react';
import { NavigationRail } from '../../dashboard/navigation/NavigationRail';
import { generateDealerBranding, isFalConfigured } from '@/lib/api/toolGeneration';
import type { DealerBrandingOptions } from '@/lib/types/generation';
...
export const DealerBrandingTool: React.FC = () => {
    ...
    const handleGenerate = async () => {
        ...
        try {
            const options: DealerBrandingOptions = {
            ...
            const resultUrl = await generateDealerBranding(
                uploadedImage,
                options,
                (progress, status) => {
                    setGenerationProgress(progress);
                    console.log('Dealer branding status:', status);
                }
            );
            setResultImage(resultUrl);
        } catch (err) {
            console.error('Dealer branding error:', err);
            setError(err instanceof Error ? err.message : 'Branding failed. Please try again.');
        } finally {
            setIsGenerating(false);
        }
    };
...
<div className="flex-1 flex ml-0 lg:ml-16">
                <div className="w-[320px] flex-shrink-0 bg-[#111113] border-r border-white/5 flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-5">
                        ...
                        <div>
                            <label className="text-xs font-medium text-zinc-400 uppercase tracking-wide mb-2 block">Logo URL (Optional)</label>
                            <input
                                type="url"
                                value={logoUrl}
                                onChange={(e) => setLogoUrl(e.target.value)}
                                placeholder="https://yourdealership.com/logo.png"
                                className="w-full px-3 py-2.5 rounded-xl bg-white/5 border border-white/10 text-sm text-white placeholder:text-zinc-600 focus:outline-none focus:border-emerald-500/50"
                            />
                            <p className="text-xs text-zinc-600 mt-1">Leave empty for text-only branding</p>
                        </div>

                        <div>
                            <label className="text-xs font-medium text-zinc-400 uppercase tracking-wide mb-3 block">Position</label>
                            <div className="grid grid-cols-3 gap-2">
                                {POSITIONS.slice(0, 3).map((pos) => (
                                    <button
                                        key={pos.id}
                                        onClick={() => setPosition(pos.id as typeof position)}
                                        className={`p-2.5 rounded-xl text-center transition-all ${position === pos.id ? 'bg-emerald-500/20 text-emerald-300 ring-1 ring-emerald-500/50' : 'bg-white/5 text-zinc-400 hover:bg-white/10'}`}
                                    >
                        ...
                        <div>
                            <label className="text-xs font-medium text-zinc-400 uppercase tracking-wide mb-2 block">Opacity: {opacity}%</label>
                            <input
                                type="range"
                                min="20"
                                max="100"
                        ...
                        <div className="flex items-center justify-between p-3 rounded-xl bg-white/[0.02] border border-white/5">
                            <div>
                                <p className="text-sm text-white font-medium">Add Watermark</p>
                                <p className="text-xs text-zinc-500">Subtle text overlay</p>
                            </div>
                            <button
                                onClick={() => setAddWatermark(!addWatermark)}
                                className={`w-11 h-6 rounded-full transition-colors ${addWatermark ? 'bg-emerald-500' : 'bg-white/10'}`}
                            >
                                <div className={`w-5 h-5 rounded-full bg-white shadow transition-transform ${addWatermark ? 'translate-x-5' : 'translate-x-0.5'}`} />
                            </button>
                        </div>

                        {addWatermark && (
                            <div>
                                <label className="text-xs font-medium text-zinc-400 uppercase tracking-wide mb-2 block">Watermark Text</label>
                                <input
                                    type="text"
                                    value={watermarkText}
                                    onChange={(e) => setWatermarkText(e.target.value)}
                                    placeholder="e.g., AutoMax Motors | (555) 123-4567"
                                    className="w-full px-3 py-2.5 rounded-xl bg-white/5 border border-white/10 text-sm text-white placeholder:text-zinc-600 focus:outline-none focus:border-emerald-500/50"
                                />
                            </div>
                        )}
                        ...
                        <div className="p-3 rounded-xl bg-emerald-500/5 border border-emerald-500/10">
                            <p className="text-xs text-emerald-300/80 leading-relaxed">
                                <strong className="text-emerald-300">Brand Your Listings:</strong> Add your dealership logo and contact info to all vehicle photos. Professional, consistent branding across listings.
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="p-4 border-t border-white/5">
                        ...
                        >
                            {isGenerating ? <><Loader2 size={18} className="animate-spin" />Branding {generationProgress}%</> : <><Sparkles size={18} />Add Branding</>}
                        </button>
                ...
                <div className="flex-1 flex flex-col bg-[#0a0a0b]">
                    <div className="h-14 flex items-center justify-between px-6 border-b border-white/5">
                        <span className="text-sm text-zinc-500">{resultImage ? `Branding: ${position}` : 'Preview'}</span>
                        {resultImage && (
                    ...
                    <div className="flex-1 flex items-center justify-center p-8">
                        {error ? (
                            <div className="text-center">
                                ...
                                <p className="text-red-400 text-sm font-medium mb-2">Branding Failed</p>
                            ...
                            <div className="text-center">
                                ...
                                <p className="text-xs text-zinc-600 mt-1">Add your dealer branding</p>
                            </div>
                        ) : isGenerating ? (
                            <div className="text-center">
                                ...
                                <p className="text-zinc-400 font-medium">Adding branding...</p>
                            </div>
                        ) : (
                            <div className="relative rounded-2xl overflow-hidden shadow-2xl">
                                <img src={resultImage || imagePreview} alt="Preview" className="max-w-full max-h-[calc(100vh-180px)] object-contain" />
                                {resultImage && <div className="absolute top-4 right-4 px-3 py-1.5 bg-emerald-500/80 backdrop-blur rounded-lg text-xs text-white font-medium flex items-center gap-1.5"><BadgeCheck size={12} />Branded</div>}
                                {!resultImage && <div className="absolute inset-0 flex items-center justify-center bg-black/40"><div className="text-center text-white"><BadgeCheck size={28} className="mx-auto mb-2 opacity-80" /><p className="text-sm font-medium">Ready to brand</p></div></div>}
                            </div>
                        )}
                    </div>
                </div>
