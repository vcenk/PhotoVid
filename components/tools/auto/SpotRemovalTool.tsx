...
import {
    ...
    AlertCircle, Droplets, Car, Undo, Paintbrush
} from 'lucide-react';
import { NavigationRail, FlyoutType } from '../../dashboard/navigation/NavigationRail';
import { FlyoutPanels } from '../../dashboard/navigation/FlyoutPanels';
import { generateSpotRemoval, isFalConfigured } from '@/lib/api/toolGeneration';
...
export const SpotRemovalTool: React.FC = () => {
    ...
    const handleGenerate = async () => {
        ...
        try {
            const resultUrl = await generateSpotRemoval(
                uploadedImage,
                maskCanvasRef.current,
                (progress, status) => {
                    setGenerationProgress(progress);
                    console.log('Spot removal status:', status);
                }
            );
            setResultImage(resultUrl);
        } catch (err) {
            console.error('Spot removal error:', err);
            setError(err instanceof Error ? err.message : 'Spot removal failed. Please try again.');
        } finally {
            setIsGenerating(false);
        }
    };
                ...
                <div className="w-[320px] flex-shrink-0 bg-[#111113] border-r border-white/5 flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-5">
                                    ...
                                    <div className="flex gap-2">
                                        {BRUSH_SIZES.map((b) => (
                                            <button
                                                ...
                                                className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${brushSize === b.size ? 'bg-cyan-500/20 text-cyan-300 ring-1 ring-cyan-500/50' : 'bg-white/5 text-zinc-400 hover:bg-white/10'}`}
                                            >
                                                {b.label}
                                            </button>
                                        ))}
                        ...
                        <div className="p-3 rounded-xl bg-cyan-500/5 border border-cyan-500/10">
                            <p className="text-xs text-cyan-300/80 leading-relaxed">
                                <strong className="text-cyan-300">Quick Clean:</strong> Paint over dirt spots, water marks, or minor imperfections. Perfect for quick touch-ups before listing.
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="p-4 border-t border-white/5">
                        <button
                            ...
                            className={`w-full py-3 rounded-xl font-semibold text-sm transition-all flex items-center justify-center gap-2 ${uploadedImage && hasMask && !isGenerating ? 'bg-cyan-600 hover:bg-cyan-500 text-white' : 'bg-white/5 text-zinc-600 cursor-not-allowed'}`}
                        >
                            {isGenerating ? <><Loader2 size={18} className="animate-spin" />Removing {generationProgress}%</> : <><Sparkles size={18} />Remove Spots</>}
                        </button>
                ...
                <div className="flex-1 flex flex-col bg-[#0a0a0b]">
                    <div className="h-14 flex items-center justify-between px-6 border-b border-white/5">
                        <span className="text-sm text-zinc-500 flex items-center gap-2">
                            {canvasReady && !resultImage && <><Paintbrush size={14} />Paint over spots to remove</>}
                            {resultImage && 'Spots Removed'}
                    ...
                    <div className="flex-1 flex items-center justify-center p-8">
                        {error ? (
                            <div className="text-center">
                                ...
                                <p className="text-red-400 text-sm font-medium mb-2">Spot Removal Failed</p>
                            ...
                            <div className="text-center">
                                ...
                                <p className="text-zinc-500 text-sm">Upload a vehicle photo to remove spots</p>
                                <p className="text-xs text-zinc-600 mt-1">Clean up dirt, water marks, and minor blemishes</p>
                            </div>
                        ) : isGenerating ? (
                            <div className="text-center">
                                ...
                                <p className="text-zinc-400 font-medium">Removing spots...</p>
                            </div>
                        ) : resultImage ? (
                            <div className="relative rounded-2xl overflow-hidden shadow-2xl">
                                <img src={resultImage} alt="Result" className="max-w-full max-h-[calc(100vh-180px)] object-contain" />
                                <div className="absolute top-4 right-4 px-3 py-1.5 bg-cyan-500/80 backdrop-blur rounded-lg text-xs text-white font-medium flex items-center gap-1.5"><Droplets size={12} />Spots Removed</div>
                            </div>
                        ) : (
                            <div className="relative rounded-2xl overflow-hidden shadow-2xl">
