...
import {
    ...
    Image as ImageIcon, Check, AlertCircle, Square, ChevronLeft, ChevronRight, BookmarkPlus
} from 'lucide-react';
...
const FloorReplacementToolInner: React.FC = () => {
    ...
    const handleGenerate = async () => {
        ...
        if (!isFalConfigured()) {
            ...
            setTimeout(async () => {
                ...
                try { await addAsset(mockUrl, 'image', 'Floor Replacement Result'); setSavedToLibrary(true); } catch {}
        ...
        try {
            ...
            try { await addAsset(resultUrl, 'image', 'Floor Replacement Result'); setSavedToLibrary(true); } catch {}
        } catch (err) {
                ...
                <div className="flex-1 flex flex-col bg-[#0a0a0b]">
                        ...
                        {resultImage && (
                            <div className="flex items-center gap-2">
                                ...
                                {savedToLibrary ? (
                                    ...
                                    <button onClick={async () => { if (resultImage) { try { await addAsset(resultImage, 'image', 'Floor Replacement Result'); setSavedToLibrary(true); } catch {} } }} className="px-3 py-1.5 text-xs font-medium text-zinc-400 hover:text-white bg-white/5 hover:bg-white/10 rounded-lg transition-colors flex items-center gap-1.5"><BookmarkPlus size={14} />Save to Library</button>
                                )}
                                <button onClick={() => resultImage && downloadFile(resultImage, `floor-replaced-${Date.now()}.jpg`)} className="px-3 py-1.5 text-xs font-medium text-white bg-amber-600 hover:bg-amber-500 rounded-lg transition-colors flex items-center gap-1.5"><Download size={14} />Download</button>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="flex-1 flex items-center justify-center p-8">
                        {error ? (
                            ...
                            <BeforeAfterSlider before={imagePreview} after={resultImage} label={style} />
                        ) : (
                            <div className="relative rounded-2xl overflow-hidden shadow-2xl">
                                <img src={imagePreview} alt="Preview" className="max-w-full max-h-[calc(100vh-180px)] object-contain" />
                                <div className="absolute inset-0 flex items-center justify-center bg-black/40"><div className="text-center text-white"><Square size={28} className="mx-auto mb-2 opacity-80" /><p className="text-sm font-medium">Ready to replace floor</p></div></div>
                            </div>
                        )}
                    </div>
                </div>
