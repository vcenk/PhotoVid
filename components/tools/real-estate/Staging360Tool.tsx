...
import {
    ...
    Image as ImageIcon, Check, AlertCircle, Compass, Crown, BookmarkPlus
} from 'lucide-react';
import { NavigationRail } from '../../dashboard/navigation/NavigationRail';
import { AssetProvider, useAssets } from '@/lib/store/contexts/AssetContext';
import { generate360Staging, isFalConfigured } from '@/lib/api/toolGeneration';
import { downloadFile } from '@/lib/utils';
import type { Staging360Options } from '@/lib/types/generation';
...
const STYLES = [
    ...
    { id: 'scandinavian', name: 'Scandinavian', desc: 'Light and cozy Nordic style' },
    { id: 'luxury', name: 'Luxury', desc: 'High-end elegant furnishings' },
    { id: 'farmhouse', name: 'Farmhouse', desc: 'Warm rustic charm' },
];

const Staging360ToolInner: React.FC = () => {
    ...
    const [roomType, setRoomType] = useState<'living-room' | 'bedroom' | 'kitchen' | 'bathroom'>('living-room');
    const [style, setStyle] = useState<'modern' | 'scandinavian' | 'luxury' | 'farmhouse'>('modern');
    ...
    const handleGenerate = async () => {
        ...
        if (!isFalConfigured()) {
            ...
            setTimeout(async () => {
                ...
                const mockUrl = 'https://images.unsplash.com/photo-1586023492125-27b2c045efd7?w=1200&h=600&fit=crop';
                setResultImage(mockUrl);
                try { await addAsset(mockUrl, 'image', '360 Staging Result'); setSavedToLibrary(true); } catch {}
        ...
        try {
            const options: Staging360Options = { style, roomType };
            const resultUrl = await generate360Staging(
                uploadedImage,
                options,
                (progress, status) => {
                    setGenerationProgress(progress);
                    console.log('360 staging status:', status);
                }
            );
            setResultImage(resultUrl);
            try { await addAsset(resultUrl, 'image', '360 Staging Result'); setSavedToLibrary(true); } catch {}
        } catch (err) {
            console.error('360 staging error:', err);
            setError(err instanceof Error ? err.message : '360° staging failed. Please try again.');
        } finally {
            setIsGenerating(false);
        }
    };
...
<div className="flex-1 flex ml-0 lg:ml-16">
                <div className="w-[360px] flex-shrink-0 bg-[#111113] border-r border-white/5 flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-5">
                        <div>
                            <label className="text-xs font-medium text-zinc-400 uppercase tracking-wide mb-2 block">360° Panorama</label>
                            {!imagePreview ? (
                                <div
                                ...
                                >
                                    ...
                                    <p className="text-sm text-zinc-400">Drop 360° panorama photo</p>
                                    <p className="text-xs text-zinc-600 mt-1">Equirectangular format</p>
                                </div>
                            ) : (
                                <div className="relative rounded-xl overflow-hidden group">
                                    <img src={imagePreview} alt="Photo" className="w-full h-24 object-cover" />
                        ...
                        <div>
                            <label className="text-xs font-medium text-zinc-400 uppercase tracking-wide mb-3 block">Room Type</label>
                ...
                <div className="flex-1 flex flex-col bg-[#0a0a0b]">
                    <div className="h-14 flex items-center justify-between px-6 border-b border-white/5">
                        <span className="text-sm text-zinc-500">{resultImage ? `Staged 360°: ${selectedStyle?.name}` : 'Preview'}</span>
                        {resultImage && (
                    ...
                    <div className="flex-1 flex items-center justify-center p-8">
                        {error ? (
                            ...
                            <div className="text-center">
                                ...
                                <p className="text-zinc-500 text-sm">Upload a 360° panorama to stage</p>
                                <p className="text-xs text-zinc-600 mt-1">Use equirectangular format for best results</p>
                            </div>
                        ) : isGenerating ? (
                            <div className="text-center">
...
export const Staging360Tool: React.FC = () => (
    <AssetProvider>
        <Staging360ToolInner />
    </AssetProvider>
);
