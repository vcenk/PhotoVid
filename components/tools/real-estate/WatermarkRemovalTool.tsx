...
import {
    ...
    Image as ImageIcon, Check, AlertCircle, Ban, BookmarkPlus
} from 'lucide-react';
...
const WatermarkRemovalToolInner: React.FC = () => {
    ...
    const handleGenerate = async () => {
        ...
        if (!isFalConfigured()) {
            ...
            setTimeout(async () => {
                ...
                try { await addAsset(mockUrl, 'image', 'Watermark Removal Result'); setSavedToLibrary(true); } catch {}
        ...
        try {
            ...
            try { await addAsset(resultUrl, 'image', 'Watermark Removal Result'); setSavedToLibrary(true); } catch {}
        } catch (err) {
                        ...
                        {resultImage && (
                            <div className="flex items-center gap-2">
                                ...
                                {savedToLibrary ? (
                                    ...
                                    <button onClick={async () => { if (resultImage) { try { await addAsset(resultImage, 'image', 'Watermark Removal Result'); setSavedToLibrary(true); } catch {} } }} className="px-3 py-1.5 text-xs font-medium text-zinc-400 hover:text-white bg-white/5 hover:bg-white/10 rounded-lg transition-colors flex items-center gap-1.5"><BookmarkPlus size={14} />Save to Library</button>
                                )}
                                <button onClick={() => resultImage && downloadFile(resultImage, `watermark-removed-${Date.now()}.jpg`)} className="px-3 py-1.5 text-xs font-medium text-white bg-red-600 hover:bg-red-500 rounded-lg transition-colors flex items-center gap-1.5"><Download size={14} />Download</button>
                            </div>
                        )}
                    </div>
